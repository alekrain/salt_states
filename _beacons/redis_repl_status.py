...
# SaltStack Beacon
#
# NAME: _beacons/redis_repl_status.py
...
__virtualname__ = 'redis_repl_status'
...
def _query_redis(config):
    ...
    Redis = redis.Redis(host=host, port=port, db=0, socket_timeout=5)
    return Redis.info()


def beacon(config):
    '''
    Send back data from redis info and sentinel commands.
